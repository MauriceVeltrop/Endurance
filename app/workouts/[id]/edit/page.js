...
function hydrateSelectedExercisesFromWorkout(workout) {
  ...
  return exercises
    ...
    .map((exercise, index) => {
      const group =
        ...
        (Array.isArray(exercise.muscle_groups) ? exercise.muscle_groups[0] : null) ||
        "Chest";
      ...
      return {
        ...
        exercise: {
          id: exercise.id || exercise.exercise_id || makeLocalId("snapshot"),
          source: exercise.source || "snapshot",
        ...
        sets: (sets.length ? sets : defaultSets()).map((set, setIndex) => ({
          id: makeLocalId("set"),
          set_number: setIndex + 1,
          reps: set?.reps === null || set?.reps === undefined ? "" : String(set.reps),
          weight_kg: set?.weight_kg === null || set?.weight_kg === undefined ? "" : String(set.weight_kg),
          rest_seconds: set?.rest_seconds === null || set?.rest_seconds === undefined ? "" : String(set.rest_seconds ?? "90"),
        })),
      };
    });
}
  ...
  async function loadAccess() {
    try {
      ...
      const { data: workoutRow, error: workoutError } = await supabase
        .from("workouts")
        .select("id,creator_id,sport_id,title,description,workout_type,level,duration_min,structure,visibility")
        .eq("id", workoutId)
        .maybeSingle();

      if (workoutError) throw workoutError;
      if (!workoutRow) throw new Error("Workout not found.");
      ...
      if (!canEdit) throw new Error("You cannot edit this workout.");

      const structure = workoutRow.structure || {};
      const hydratedExercises = hydrateSelectedExercisesFromWorkout(workoutRow);

      setForm({
        sport_id: workoutRow.sport_id || firstWorkoutSport || "strength_training",
        title: workoutRow.title || `${firstName(profileRow)} Strength Workout`,
        description: workoutRow.description || "",
        workout_type: workoutRow.workout_type || "strength",
        level: workoutRow.level || "all levels",
        duration_min: workoutRow.duration_min === null || workoutRow.duration_min === undefined ? "" : String(workoutRow.duration_min),
        visibility: workoutRow.visibility || "private",
      });
      setMethod(structure.method || (workoutRow.workout_type === "wizard" ? "wizard" : "manual"));
  ...
  async function saveWorkout(event) {
    ...
    try {
      ...
      try {
        ...
        const existingIds = (existingWorkoutExercises || []).map((row) => row.id);
        if (existingIds.length) {
          await supabase.from("workout_exercise_sets").delete().in("workout_exercise_id", existingIds);
        }
    ...
    <main style={styles.page}>
      <section style={styles.shell}>
        ...
        <header style={styles.header}>
          <div style={styles.kicker}>Workout Editor</div>
          <h1 style={styles.title}>Edit your workout.</h1>
