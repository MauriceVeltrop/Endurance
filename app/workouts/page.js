      ...
      <section style={styles.shell}>
        ...
        <header style={styles.header}>
          ...
          <div style={styles.titleRow}>
            ...
            <button type="button" onClick={() => router.push("/workouts/new")} style={styles.primaryButton}>
              + Workout
            </button>
          </div>
          <p style={styles.subtitle}>
            Save strength, HYROX, CrossFit and bootcamp workouts so they can be connected to training sessions.
          </p>
        </header>
        ...
        {!message && !workouts.length ? (
          <section style={styles.emptyCard}>
            ...
            <p>Create reusable workout structures for strength and hybrid trainings.</p>
            <button type="button" onClick={() => router.push("/workouts/new")} style={styles.primaryButton}>
        ...
        {workouts.length ? (
          <section style={styles.grid}>
            {workouts.map((workout) => {
              const count = Array.isArray(workout.structure?.exercises) ? workout.structure.exercises.length : 0;
              ...
              return (
                <article key={workout.id} style={styles.card}>
                  ...
                  <div style={styles.facts}>
                    ...
                    <span>{count ? `${count} blocks` : "No blocks yet"}</span>
                  </div>

                  <button type="button" onClick={() => router.push(`/workouts/new?from=${workout.id}`)} style={styles.secondaryButton}>
                    Use as template
                  </button>
                </article>
              );
...
const styles = {
  ...
  secondaryButton: { minHeight: 42, borderRadius: 999, border: "1px solid rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.08)", color: "white", padding: "0 14px", fontWeight: 950, cursor: "pointer", justifySelf: "start" },
  ...
  facts: { display: "grid", gap: 7, color: "rgba(255,255,255,0.68)", fontWeight: 750 },
};
