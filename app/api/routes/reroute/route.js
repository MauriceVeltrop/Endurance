...
const ORS_BASE = "https://api.heigit.org/routing/2/directions";
...
function cleanProviderError(text){
  ...
  if (raw.includes("<html") || raw.includes("<!DOCTYPE") || raw.includes("nginx")) {
    return "Routing provider returned an HTML error page. Check endpoint URL and API key.";
  }
  return raw.slice(0, 600);
}
export async function POST(request){
  try{
    const apiKey=process.env.OPENROUTE_API_KEY||process.env.OPENROUTESERVICE_API_KEY||process.env.ORS_API_KEY||process.env.NEXT_PUBLIC_OPENROUTE_API_KEY;
    if(!apiKey) return NextResponse.json({error:"Missing OPENROUTE_API_KEY, OPENROUTESERVICE_API_KEY or ORS_API_KEY environment variable."},{status:500});
    ...
    if(!res.ok){const text=await res.text(); return NextResponse.json({error:`OpenRouteService failed (${res.status}): ${cleanProviderError(text)}`},{status:res.status});}
