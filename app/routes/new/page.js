    ...
    try {
      ...
      if (templateId) {
        const { data: template, error: templateError } = await supabase
          .from("routes")
          .select("sport_id,title,description,visibility,distance_km,elevation_gain_m,gpx_file_url")
          .eq("id", templateId)
          .maybeSingle();
        ...
        {checking ? (
          ...
          <form onSubmit={saveRoute} style={styles.formCard}>
            ...
            <label style={styles.field}>
              ...
              <input value={form.gpx_file_url} onChange={(event) => updateForm("gpx_file_url", event.target.value)} placeholder="Later replaced by upload" style={styles.input} />
            </label>
