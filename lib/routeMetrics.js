  ...
  const raw = Array.isArray(routePoints)
    ...
    : Array.isArray(routePoints?.points)
      ...
      : Array.isArray(routePoints?.geometry_points)
        ...
        : Array.isArray(routePoints?.coordinates)
  ...
  return raw
    .map((p) => {
      if (Array.isArray(p)) {
        ...
        const looksLikeGeoJson = Math.abs(first) > 90 && Math.abs(second) <= 90;
        return {
          lat: looksLikeGeoJson ? second : first,
          lon: looksLikeGeoJson ? first : second,
