...
export function calculateRouteMetrics(routePoints){
  const pts=normalizeRoutePoints(routePoints);
  let d=0,gain=0,loss=0,max=null;
  ...
  for(let i=0;i<pts.length;i++){
    ...
    if(i>0){
      ...
      if(Number.isFinite(a)&&Number.isFinite(b)){
        ...
        const diff=b-a;
        const maxPlausibleStep=Math.max(5,horizontal*0.35);
        if(Math.abs(diff)>maxPlausibleStep)continue;

        if(diff>0){
          pendingUp+=diff;
          if(pendingDown>=minClimbStep)loss+=pendingDown;
          pendingDown=0;
        }else if(diff<0){
          pendingDown+=Math.abs(diff);
          if(pendingUp>=minClimbStep)gain+=pendingUp;
          pendingUp=0;
        }
      }
    }
  ...
  return{
    distance_km:Number((d/1000).toFixed(2)),
