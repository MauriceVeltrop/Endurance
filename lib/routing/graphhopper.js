...
const RUNNING_CUSTOM_MODEL = {
  // Lower distance influence makes GraphHopper more willing to take a small detour
  // when the route avoids poor running surfaces.
  distance_influence: 50,
  priority: [
    // Hard avoid.
    { if: "road_class == STEPS", multiply_by: 0.01 },

    // Surface is the main steering input for Running.
    { if: "surface == MUD", multiply_by: 0.05 },
    { if: "surface == SAND", multiply_by: 0.08 },
    { if: "surface == GRASS", multiply_by: 0.15 },
    { if: "surface == GROUND", multiply_by: 0.35 },
    { if: "surface == DIRT", multiply_by: 0.35 },
    { if: "surface == EARTH", multiply_by: 0.35 },
    { if: "surface == GRAVEL", multiply_by: 0.55 },
    { if: "surface == FINE_GRAVEL", multiply_by: 0.7 },

    // Track itself is neutral, but rough track types are discouraged.
    { if: "track_type == GRADE5", multiply_by: 0.25 },
    { if: "track_type == GRADE4", multiply_by: 0.4 },
    { if: "track_type == GRADE3", multiply_by: 0.65 },
    { if: "track_type == GRADE2", multiply_by: 0.85 },
  ],
};
...
function buildGraphHopperRequest(points, includeDetails = true, includeCustomModel = true) {
  const body = {
    profile: "foot",
    locale: "nl",
  ...
  if (includeCustomModel) {
    body.custom_model = RUNNING_CUSTOM_MODEL;
...
async function postGraphHopper({ apiKey, body, signal, includeDetails, includeCustomModel }) {
  const requestSummary = {
    ...
    include_custom_model: includeCustomModel,
    ch_disabled: body?.["ch.disable"] === true,
    custom_model_rules: Array.isArray(body?.custom_model?.priority) ? body.custom_model.priority.length : 0,
  ...
  try {
    ...
    try {
      data = await postGraphHopper({
        apiKey,
        body: buildGraphHopperRequest(points, true, true),
        signal: controller.signal,
        includeDetails: true,
        includeCustomModel: true,
      });
      customModelUsed = true;
    } catch (error) {
      console.warn("GraphHopper custom model failed; retrying baseline with details", {
        message: error?.message || "failed",
      });
      ...
      try {
        data = await postGraphHopper({
          apiKey,
          body: buildGraphHopperRequest(points, true, false),
          signal: controller.signal,
          includeDetails: true,
          includeCustomModel: false,
        });
      } catch (detailsError) {
        console.warn("GraphHopper route with details failed; retrying baseline without details", {
          message: detailsError?.message || "failed",
        });

        data = await postGraphHopper({
          apiKey,
          body: buildGraphHopperRequest(points, false, false),
          signal: controller.signal,
    ...
    const candidate = {
      ...
      profile: "foot",
      preference: customModelUsed ? "running-custom" : "recommended",
      points: geometry,
      distance,
      duration,
      ...
      graphhopper_baseline: !customModelUsed,
      graphhopper_custom_model: customModelUsed,
    };
