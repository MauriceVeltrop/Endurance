...
export function buildRouteGeometryInsert({ routeId, routePoints, sourceType = "gpx" }) {
  const geometry = normalizeRoutePoints(routePoints);
...
export function inferRouteSourceType(methodOrPayload, routePoints = null) {
  ...
  const source = String(routePoints?.source || methodOrPayload?.source || "").toLowerCase();
