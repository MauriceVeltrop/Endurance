import { calculateRouteMetrics, normalizeRoutePoints, simplifyRoutePoints } from "../routeMetrics";

export function segmentKey(a, b, sportId = "") {
  const p1 = normalizeRoutePoints([a])[0];
  const p2 = normalizeRoutePoints([b])[0];
  if (!p1 || !p2) return "";
  return [
    String(sportId || ""),
    p1.lat.toFixed(6),
    p1.lon.toFixed(6),
    p2.lat.toFixed(6),
    p2.lon.toFixed(6),
  ].join("|");
}

export function getControlSegments(controlPoints, sportId = "") {
  const points = normalizeRoutePoints(controlPoints);
  const segments = [];

  for (let index = 0; index < points.length - 1; index += 1) {
    const from = points[index];
    const to = points[index + 1];
    segments.push({
      index,
      from,
      to,
      key: segmentKey(from, to, sportId),
      control: [from, to],
    });
  }

  return segments;
}

export function compactRouteGeometry(points, maxPoints = 1400) {
  const normalized = normalizeRoutePoints(points);
  const simplified = simplifyRoutePoints(normalized, normalized.length > 500 ? 4 : 1.5);

  if (simplified.length <= maxPoints) return simplified;

  const step = Math.ceil(simplified.length / maxPoints);
  const output = simplified.filter((_, index) => index % step === 0);
  const last = simplified[simplified.length - 1];

  if (last && output[output.length - 1] !== last) output.push(last);

  return output;
}

export function joinSegmentGeometries(segments) {
  const output = [];

  (Array.isArray(segments) ? segments : []).forEach((segment) => {
    const geometry = normalizeRoutePoints(segment?.geometry || segment?.points || segment);

    geometry.forEach((point) => {
      const previous = output[output.length - 1];
      if (
        previous &&
        Math.abs(Number(previous.lat) - Number(point.lat)) <= 1e-6 &&
        Math.abs(Number(previous.lon) - Number(point.lon)) <= 1e-6
      ) {
        return;
      }

      output.push(point);
    });
  });

  return compactRouteGeometry(output);
}

function pointDistance(a, b) {
  if (!a || !b) return Infinity;
  return Math.hypot(Number(a.lat) - Number(b.lat), Number(a.lon) - Number(b.lon));
}

function nearestGeometryIndex(target, geometry, startIndex = 0) {
  const needle = normalizeRoutePoints([target])[0];
  const points = normalizeRoutePoints(geometry);
  if (!needle || !points.length) return -1;

  let bestIndex = -1;
  let bestDistance = Infinity;
  for (let index = Math.max(0, Number(startIndex) || 0); index < points.length; index += 1) {
    const distance = pointDistance(needle, points[index]);
    if (distance < bestDistance) {
      bestDistance = distance;
      bestIndex = index;
    }
  }
  return bestIndex;
}

function segmentFromDefinition(definition, geometry, fromIndex, toIndex, source = "hydrated-route-segment") {
  const start = Math.max(0, Number(fromIndex) || 0);
  const end = Math.max(start + 1, Number(toIndex) || start + 1);
  let segmentGeometry = normalizeRoutePoints(geometry).slice(start, end + 1);

  if (segmentGeometry.length < 2) {
    segmentGeometry = normalizeRoutePoints(definition.control);
  }

  const first = segmentGeometry[0];
  const last = segmentGeometry[segmentGeometry.length - 1];
  const from = definition.from;
  const to = definition.to;

  if (from && first && pointDistance(from, first) > 1e-7) {
    segmentGeometry = [from, ...segmentGeometry];
  }
  if (to && last && pointDistance(to, last) > 1e-7) {
    segmentGeometry = [...segmentGeometry, to];
  }

  return {
    ...definition,
    geometry: segmentGeometry,
    points: segmentGeometry,
    routed: true,
    status: "snapped",
    source,
    hydrated: true,
    updated_at: new Date().toISOString(),
  };
}

export function hydrateSegmentsFromRoutePayload({ routePayload, controlPoints, sportId = "" } = {}) {
  const controls = normalizeRoutePoints(controlPoints?.length ? controlPoints : routePayload?.control_points || routePayload?.waypoints);
  const definitions = getControlSegments(controls, sportId);
  if (!definitions.length) return [];

  const existingSegments = Array.isArray(routePayload?.route_segments)
    ? routePayload.route_segments
    : Array.isArray(routePayload?.segments)
      ? routePayload.segments
      : [];

  if (existingSegments.length) {
    const byKey = new Map();
    existingSegments.forEach((segment) => {
      const geometry = normalizeRoutePoints(segment?.geometry || segment?.points);
      const from = segment?.from || segment?.control?.[0];
      const to = segment?.to || segment?.control?.[1];
      const key = segment?.key || segmentKey(from, to, sportId);
      if (key && geometry.length >= 2) {
        byKey.set(key, {
          ...segment,
          key,
          geometry,
          points: geometry,
          routed: segment?.routed !== false,
          status: segment?.status || "snapped",
          hydrated: true,
        });
      }
    });

    const hydrated = definitions.map((definition) => {
      const cached = byKey.get(definition.key);
      if (cached?.geometry?.length >= 2) {
        return { ...definition, ...cached, control: definition.control, from: definition.from, to: definition.to, index: definition.index };
      }
      return segmentFromDefinition(definition, definition.control, 0, 1, "hydrated-control-fallback");
    });

    if (hydrated.length === definitions.length) return hydrated;
  }

  const geometry = normalizeRoutePoints(routePayload?.points?.length ? routePayload.points : routePayload?.geometry_points);
  if (geometry.length < 2) {
    return definitions.map((definition) => segmentFromDefinition(definition, definition.control, 0, 1, "hydrated-control-fallback"));
  }

  let searchFrom = 0;
  return definitions.map((definition, index) => {
    const isLast = index === definitions.length - 1;
    const fromIndex = index === 0 ? nearestGeometryIndex(definition.from, geometry, searchFrom) : searchFrom;
    let toIndex = nearestGeometryIndex(definition.to, geometry, Math.max(0, fromIndex));

    if (isLast) {
      const lastGeometryIndex = geometry.length - 1;
      const endDistance = pointDistance(definition.to, geometry[lastGeometryIndex]);
      if (endDistance <= 0.0002 || toIndex < fromIndex) {
        toIndex = lastGeometryIndex;
      }
    }

    if (fromIndex < 0 || toIndex <= fromIndex) {
      const fallback = segmentFromDefinition(definition, definition.control, 0, 1, "hydrated-control-fallback");
      searchFrom = Math.max(searchFrom, fromIndex + 1);
      return fallback;
    }

    searchFrom = toIndex;
    return segmentFromDefinition(definition, geometry, fromIndex, toIndex);
  });
}

function addPercentMapToMeters(target, percentMap, meters) {
  Object.entries(percentMap || {}).forEach(([key, percent]) => {
    const value = (Number(percent || 0) / 100) * Number(meters || 0);
    if (!Number.isFinite(value) || value <= 0) return;
    target[key] = (target[key] || 0) + value;
  });
}

function aggregateRouteQuality(segments, metrics, sportId = "", controlPoints = []) {
  const totalDistanceMeters = Math.max(1, Number(metrics?.distance_km || 0) * 1000);
  const directMetrics = calculateRouteMetrics(normalizeRoutePoints(controlPoints));
  const directDistanceMeters = Math.max(1, Number(directMetrics?.distance_km || 0) * 1000);
  const detourFactor = totalDistanceMeters / directDistanceMeters;
  const surfaces = {};
  const waytypes = {};
  let scoreMeters = 0;
  let suitableMeters = 0;
  let unsuitableMeters = 0;
  let unknownMeters = 0;
  let candidateCount = 0;
  let segmentDetourMeters = 0;
  const candidateSegments = [];

  (Array.isArray(segments) ? segments : []).forEach((segment) => {
    const geometry = normalizeRoutePoints(segment?.geometry || segment?.points || segment?.control);
    const segmentMetrics = calculateRouteMetrics(geometry);
    const meters = Math.max(1, Number(segmentMetrics?.distance_km || 0) * 1000);
    const quality = segment?.quality || {};

    if (Number.isFinite(Number(quality.score))) {
      scoreMeters += Number(quality.score) * meters;
    }

    suitableMeters += (Number(quality.suitable_percent || quality.paved_percent || 0) / 100) * meters;
    unsuitableMeters += (Number(quality.unsuitable_percent || 0) / 100) * meters;
    unknownMeters += (Number(quality.unknown_percent || 0) / 100) * meters;
    candidateCount += Number(quality.candidates || 0);
    if (Number.isFinite(Number(quality.detour))) {
      segmentDetourMeters += Number(quality.detour) * meters;
    }
    addPercentMapToMeters(surfaces, quality.surfaces, meters);
    addPercentMapToMeters(waytypes, quality.waytypes, meters);

    if (Array.isArray(quality.candidate_summary) && quality.candidate_summary.length) {
      candidateSegments.push({
        segment_index: Number(segment?.index || 0),
        distance_km: Number(segmentMetrics?.distance_km || 0),
        selected_score: Number.isFinite(Number(quality.score)) ? Number(quality.score) : null,
        selected_detour: Number.isFinite(Number(quality.detour)) ? Number(quality.detour) : null,
        selected_surfaces: quality.surfaces || {},
        selected_waytypes: quality.waytypes || {},
        selected_paved_percent: Number.isFinite(Number(quality.paved_percent)) ? Number(quality.paved_percent) : null,
        selected_raw_paved_percent: Number.isFinite(Number(quality.raw_paved_percent)) ? Number(quality.raw_paved_percent) : null,
        selected_inferred_paved_unknown_percent: Number.isFinite(Number(quality.inferred_paved_unknown_percent)) ? Number(quality.inferred_paved_unknown_percent) : null,
        selected_suspicious_track_path_percent: Number.isFinite(Number(quality.suspicious_track_path_percent)) ? Number(quality.suspicious_track_path_percent) : null,
        debug: quality.debug || null,
        candidates: quality.candidate_summary.slice(0, 6),
      });
    }
  });

  const suitabilityScore = scoreMeters > 0 ? Math.round(scoreMeters / totalDistanceMeters) : null;

  return {
    sport_id: sportId || null,
    suitability_score: suitabilityScore,
    surface_quality: {
      ideal_ratio: Math.max(0, Math.min(1, suitableMeters / totalDistanceMeters)),
      acceptable_ratio: 0,
      avoid_ratio: Math.max(0, Math.min(1, unsuitableMeters / totalDistanceMeters)),
      unknown_ratio: Math.max(0, Math.min(1, unknownMeters / totalDistanceMeters)),
    },
    surfaces,
    waytypes,
    detour_factor: Number.isFinite(detourFactor) ? Number(detourFactor.toFixed(3)) : 1,
    average_segment_detour_factor: segmentDetourMeters > 0 ? Number((segmentDetourMeters / totalDistanceMeters).toFixed(3)) : null,
    candidates_considered: candidateCount,
    candidate_segments: candidateSegments
      .sort((a, b) => Number(a.selected_score ?? 999) - Number(b.selected_score ?? 999))
      .slice(0, 8),
    segment_based: true,
  };
}

function serializeRouteSegments(segments) {
  return (Array.isArray(segments) ? segments : []).map((segment) => {
    const geometry = normalizeRoutePoints(segment?.geometry || segment?.points || segment?.control);
    return {
      index: segment?.index,
      key: segment?.key,
      from: segment?.from,
      to: segment?.to,
      control: normalizeRoutePoints(segment?.control || [segment?.from, segment?.to]),
      geometry,
      points: geometry,
      routed: segment?.routed !== false,
      status: segment?.status || (segment?.routed === false ? "failed" : "snapped"),
      source: segment?.source || null,
      fallback_reason: segment?.fallback_reason || null,
      quality: segment?.quality || null,
      hydrated: Boolean(segment?.hydrated),
      updated_at: segment?.updated_at || null,
    };
  });
}

export function buildRoutePayloadFromSegments({ segments, controlPoints, source = "segmented-routing", routeQuality = null, sportId = "" }) {
  const safeSegments = Array.isArray(segments) ? segments : [];
  const geometry = joinSegmentGeometries(safeSegments);
  const waypoints = normalizeRoutePoints(controlPoints);
  const metrics = calculateRouteMetrics(geometry);
  const failedSegments = safeSegments.filter((segment) => segment?.routed === false).length;
  const snappedSegments = safeSegments.filter((segment) => segment?.routed !== false && segment?.status !== "pending").length;

  return {
    source,
    points: geometry,
    waypoints,
    control_points: waypoints,
    route_segments: serializeRouteSegments(safeSegments),
    point_count: geometry.length,
    segment_count: safeSegments.length,
    snapped_segment_count: snappedSegments,
    failed_segment_count: failedSegments,
    segment_status: safeSegments.map((segment) => ({
      index: segment?.index,
      key: segment?.key,
      routed: segment?.routed !== false,
      status: segment?.status || (segment?.routed === false ? "failed" : "snapped"),
      fallback_reason: segment?.fallback_reason || null,
    })),
    segmented: true,
    routed: failedSegments === 0,
    distance_km: metrics.distance_km || null,
    elevation_gain_m: metrics.elevation_gain_m || 0,
    elevation_loss_m: metrics.elevation_loss_m || 0,
    max_elevation_m: metrics.max_elevation_m || null,
    route_quality: routeQuality || aggregateRouteQuality(safeSegments, metrics, sportId, waypoints),
    routed_at: new Date().toISOString(),
  };
}
