...
export const METHOD_ORDER = ["draw", "upload"];
...
export async function resolveStartPlaceName(routePoints) {
  const points = normalizeRoutePoints(routePoints);
  const first = points[0];
  if (!first) return cleanRouteLocationName(routePoints?.start_location_label || routePoints?.start_location || "");

  const lat = Number(first.lat);
  const lon = Number(first.lon);
  if (!Number.isFinite(lat) || !Number.isFinite(lon)) {
    return cleanRouteLocationName(routePoints?.start_location_label || routePoints?.start_location || "");
  }

  try {
    const response = await fetch(`/api/geocode/reverse?lat=${encodeURIComponent(lat)}&lon=${encodeURIComponent(lon)}&prefer=city`);
    if (!response.ok) throw new Error("Reverse geocode failed");
    const data = await response.json();
    return pickReverseGeocodePlace(data);
  } catch (error) {
    console.warn("Could not resolve route start place", error);
    return cleanRouteLocationName(routePoints?.start_location_label || routePoints?.start_location || "");
  }
}
