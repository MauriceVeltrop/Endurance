...
export function profilesForSportConfig(sportId, requestedProfile, mode) {
  ...
  const optimization = String(mode || "balanced");

  const extras = [];
  if (optimization === "prefer_paved") {
    if (["running", "walking"].includes(normalizeSportKey(sportId))) extras.push("cycling-regular", "foot-walking");
    if (["road_cycling", "roadcycling", "cycling"].includes(normalizeSportKey(sportId))) extras.push("cycling-road", "cycling-regular");
  }
  if (optimization === "prefer_trail") {
    extras.push("foot-hiking", "cycling-mountain");
  }

  return [...new Set([requested, ...extras, ...defaults].filter(Boolean))];
}

export function preferencesForSportConfig(sportId, mode) {
  const base = routeQualityForSportConfig(sportId).preferences || ["recommended"];
  const optimization = String(mode || "balanced");

  if (optimization === "reduce_detour") {
    return [...new Set(["shortest", ...base].filter(Boolean))];
  }

  if (optimization === "prefer_paved") {
    return [...new Set(["recommended", "fastest", ...base].filter(Boolean))];
  }
