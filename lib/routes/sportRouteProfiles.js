...
export const SPORT_ROUTE_PROFILES = {
  running: {
    ...
    // Stable Running mode:
    // - use ORS foot-walking;
    // - do not use custom_model, because ORS reported this is unavailable for foot-walking;
    // - do not hard-block path, because that can remove every candidate;
    // - discourage path/unpaved only in candidate scoring.
    providerProfiles: ["foot-walking"],
    preferences: ["recommended"],
    maxDetourFactor: 10.0,
    suitableSurfaces: ["asphalt", "paved", "concrete", "paving_stones"],
    acceptableSurfaces: ["unknown"],
    unsuitableSurfaces: ["unpaved", "ground", "dirt", "earth", "gravel", "fine_gravel", "sand", "mud", "grass"],
    suitableWaytypes: ["footway", "pedestrian", "street", "road"],
    acceptableWaytypes: ["track"],
...
export function getRoutingPreferences(value) {
  return getSportRouteProfile(value).preferences || ["shortest"];
}
