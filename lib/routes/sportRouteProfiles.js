...
export const SPORT_ROUTE_PROFILES = {
  running: {
    ...
    // Unified (Trail)Running baseline:
    // - one sport instead of separate Running / Trail Running routing;
    // - always use ORS foot-walking;
    // - do not promise hard paved/unpaved steering;
    // - surface/waytype are still reported afterwards for route quality.
    ...
    acceptableSurfaces: ["unknown", "unpaved"],
    unsuitableSurfaces: ["mud", "sand"],
    suitableWaytypes: ["footway", "path", "track", "cycleway", "street", "road", "pedestrian"],
    acceptableWaytypes: ["state_road"],
    unsuitableWaytypes: ["steps"],
  },

  // Backwards-compatible alias for existing stored routes / URLs.
  trail_running: null,
...
export function normalizeSportId(value) {
  ...
  if (key === "trailrunning" || key === "trail_running") return "running";
...
export function getRoutingPreferences(value) {
  return getSportRouteProfile(value).preferences || ["recommended"];
}
