...
const distanceBetween = (a, b) => {
  ...
  const dLon = toRad(Number(b.lon ?? b.lng) - Number(a.lon ?? a.lng));
...
const smoothElevationByDistance = (rawProfile, windowMeters = 60) => {
...
export const enrichPointsWithElevationApi = async (
  points,
  { spacingMeters = 50, smoothingMeters = 60 } = {}
) => {
  if (!points || points.length < 2) return points || [];
  ...
  return smoothElevationByDistance(enriched, smoothingMeters).map((point) => ({
    lat: point.lat,
    lon: point.lon,
...
const calculateRollingGrades = (profile, windowMeters = 100) => {
...
const isLikelyClosedLoop = (
  profile,
  maxDistanceMeters = 80,
  maxElevationDiffMeters = 8
) => {
  ...
  const startEndDistance = distanceBetween(first, last);
...
const calculateGainLoss = (profile, thresholdMeters = 1.25) => {
  if (!profile || profile.length < 2) {
  ...
  for (let i = 1; i < profile.length; i++) {
    ...
    if (diff > 0) {
      ...
      if (accumulatedDescent >= thresholdMeters) {
      ...
      if (accumulatedClimb >= thresholdMeters) {
