...
export function makeElevationPolyline(routePoints, width = 320, height = 90, padding = 10) {
  const valid = getRoutePoints(routePoints)
    .filter((point) => Number.isFinite(Number(point.ele)))
    .map((point) => Number(point.ele));

  if (valid.length < 2) return "";

  const minEle = Math.min(...valid);
  const maxEle = Math.max(...valid);
  const eleRange = maxEle - minEle || 1;

  return valid
    .map((ele, index) => {
      const x = padding + (index / (valid.length - 1)) * (width - padding * 2);
      const y = padding + ((maxEle - ele) / eleRange) * (height - padding * 2);
      return `${x.toFixed(1)},${y.toFixed(1)}`;
    })
    .join(" ");
}
...
export function getElevationStats(routePoints) {
  const elevations = getRoutePoints(routePoints)
    .map((point) => Number(point.ele))
