...
export function getElevationStats(routePoints) {
  ...
  return {
    ...
    max,
    range: max - min,
    gain: Number.isFinite(Number(metrics.elevation_gain_m)) ? Math.round(Number(metrics.elevation_gain_m)) : null,
    loss: Number.isFinite(Number(metrics.elevation_loss_m)) ? Math.round(Number(metrics.elevation_loss_m)) : null,
    available: true,
    sampleCount: elevations.length,
    method: metrics.elevation_quality?.method || "unified_route_metrics",
  };
}
