...
export default function FullscreenRouteDrawPage() {
  ...
  useEffect(() => {
    async function bootstrap() {
      ...
      try {
        ...
        const isImportedGpxEdit = Boolean(draftRouteId) && (routeSource.includes("gpx") || Boolean(editDraft?.gpx_file_url));
        ...
        if (editDraft?.route_points?.points?.length) {
          ...
          const editableControlPoints = savedWaypoints.length >= 2 && !isImportedGpxRoute && savedWaypoints.length <= 24
            ? savedWaypoints
            : compactControlPoints(geometry, isImportedGpxRoute ? 6 : 14);
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(timeout);
  }, [routeSignature, sportId, isImportedEditRoute, routedPoints.length]);
