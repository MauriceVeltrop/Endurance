...
function compactControlPoints(points, maxPoints = 14) {
  ...
  if (first && (result[0]?.lat !== first.lat || result[0]?.lon !== first.lon)) {
    result.unshift(first);
  }

  if (last && (result[result.length - 1]?.lat !== last.lat || result[result.length - 1]?.lon !== last.lon)) {
    result.push(last);
  }
...
export default function FullscreenRouteDrawPage() {
  ...
  useEffect(() => {
    async function bootstrap() {
      ...
      try {
        ...
        const routeSource = String(editDraft?.route_points?.source || "").toLowerCase();
        const draftGeometryPointCount = normalizeRoutePoints(editDraft?.route_points?.points).length;
        const draftControlPointCount = normalizeRoutePoints(editDraft?.route_points?.waypoints || editDraft?.route_points?.control_points).length;
        const isImportedGpxEdit = Boolean(draftRouteId) && (
          routeSource.includes("gpx") ||
          Boolean(editDraft?.gpx_file_url) ||
          (draftGeometryPointCount > 180 && draftControlPointCount < 3)
        );
        setIsImportedEditRoute(isImportedGpxEdit);
        setGpxGeometryLocked(isImportedGpxEdit);
        ...
        if (editDraft?.route_points?.points?.length) {
          ...
          const isImportedGpxRoute = isImportedGpxEdit;
          const editableControlPoints = isImportedGpxRoute
            ? compactControlPoints(geometry, 2)
            : savedWaypoints.length >= 2 && savedWaypoints.length <= 24
              ? savedWaypoints
  ...
  async function continueToDetails() {
    ...
    if (editRouteId) {
      try {
        ...
        const normalizedRoutePoints = normalizeRoutePoints(safeRoutePoints?.points);
        const normalizedWaypoints = normalizeRoutePoints(safeRoutePoints?.waypoints || safeRoutePoints?.control_points);
  ...
  useEffect(() => {
    ...
    // Imported GPX routes already contain complete geometry. On initial edit load,
    // do not rebuild them from compact control points; that is what made mobile
    // edit feel heavy and temporarily rendered long straight provisional lines.
    // As soon as the user changes a point, loadedDraftRef is cleared and normal
    // segment rerouting resumes.
    if (isImportedEditRoute && gpxGeometryLocked && routedPoints.length >= 2) return;
    if (isImportedEditRoute && loadedDraftRef.current && routedPoints.length >= 2) return;

    const timeout = window.setTimeout(() => {
      syncRouteSegments(points, { silent: true });
    }, isImportedEditRoute ? 650 : 250);

    return () => window.clearTimeout(timeout);
  }, [routeSignature, sportId, isImportedEditRoute, gpxGeometryLocked, routedPoints.length]);
