...
export default function RouteDrawMap({
        ...
        if (!mapRef.current) {
          ...
          mapRef.current.on("click", (event) => {
            ...
            if (nativeGeometryEdit && currentPoints.length >= 2) {
              // In existing GPX/edit mode, a free map click should never append
              // a new endpoint. Only clicks on the route line create edit handles.
              return;
            }
  ...
  }, [linePoints, waypoints, onChange, routeMode, nativeGeometryEdit, dynamicHandle?.lat, dynamicHandle?.lon, dynamicHandle?.insertAt, mapZoom]);
